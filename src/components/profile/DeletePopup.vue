  ...
  <div
    ...
    <form @submit.prevent="deleteBar">
      <div class="flex flex-col justify-center">
        <h1 class="text-center">Are you sure you want to delete <span class="underline">{{ this.bar.name }}</span> bar?
        </h1>
        <br />
      </div>
      <div class="flex gap-4">
        <button class="bg-red-700 hover:bg-red-900 text-white font-bold px-4 py-2 rounded flex mx-auto" type="submit"
          @click="DeleteBarRequest">
          {{ $t("message.delete") }}
        </button>
        <button class="bg-blue-700 hover:bg-blue-900 text-white font-bold px-4 py-2 rounded flex mx-auto" type="submit"
          @click="Close">
          {{ $t("message.cancel") }}
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    async DeleteBarRequest() {
      const barId = this.bar.id
      this.$store.dispatch("deleteBar", barId).then(() => {
        location.reload()
        this.Close();
      }).catch((error) => {
        console.log(error);
